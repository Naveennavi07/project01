...
query="SELECT * FROM trafficpostlogs_db.trafficpostlogs_data"
...
FROM trafficpostlogs_db.trafficpostlogs_data
...
""",
    ...
    trafficpostlogs_db.trafficpostlogs_data WHERE search_conducted=1 GROUP BY vehicle_number ORDER BY total_searches DESC LIMIT 10""",
    ...
    ELSE '>60'
    END AS driver_age_group, AVG(is_arrested)*100.0 AS arrest_rate FROM trafficpostlogs_db.trafficpostlogs_data GROUP BY driver_age_group ORDER BY arrest_rate DESC LIMIT 1""",
    "What is the gender distribution of drivers stopped in each country":"""SELECT country_name,driver_gender,COUNT(*) AS total_stops, ROUND((COUNT(*)*100.0/SUM(COUNT(*)) OVER (PARTITION BY country_name)),2 )AS percentage FROM trafficpostlogs_db.trafficpostlogs_data
     GROUP BY country_name,driver_gender 
      ORDER BY country_name,total_stops DESC""",
    "Which race and gender combination has the highest search rate?":"""SELECT driver_race,driver_gender,AVG(search_conducted)*100 AS search_rate_percentage FROM trafficpostlogs_db.trafficpostlogs_data
    ...
    ELSE 'Night(9PM - 4AM)'
    END AS time_of_day,COUNT(*) AS total_stops FROM trafficpostlogs_db.trafficpostlogs_data
...
WHEN '30+ Min' THEN 45
END
) AS avg_stop_duration_minutes FROM trafficpostlogs_db.trafficpostlogs_data
GROUP BY violation
...
ROUND(SUM(is_arrested)*100.0/COUNT(*),2) AS arrest_rate_percentage FROM trafficpostlogs_db.trafficpostlogs_data
...
FROM
    (
        ...
        FROM
            trafficpostlogs_db.trafficpostlogs_data
        GROUP BY
            country_name, year
    ) AS yearly_stats
ORDER BY
...
SELECT
    ...
    v.violation_count
FROM
    trafficpostlogs_db.trafficpostlogs_data AS d
JOIN
    (
        SELECT
            driver_age,
...
SELECT
    ...
    t.total_stops
FROM
    trafficpostlogs_db.trafficpostlogs_data AS s
JOIN
    (
        SELECT
            EXTRACT(YEAR FROM stop_date) AS year,
